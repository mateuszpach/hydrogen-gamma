...
public class ParserImplDummy implements Parser {
    ...
    public TilesContainer parse(String variables, String expression) {
        ...
        DefaultTile t1 = new NumericTile(new NumericVariable(21.37), "A+B");
        DefaultTile t2 = new TextTile(new TextVariable("Lorem ipsum."), "C*B");
        double[][] a = {{1, 0, 1}, {0, 1, 1}, {1, 0, 1}};
        DefaultTile t3 = new MatrixTile(new MatrixVariable(a), "A+B^n: \\text{substitution #1}");
        DefaultTile t4 = new FunctionTile(new FunctionVariable("(e^{x}-sin(x))-x+\\ln{(x)}"), "A+B");
