...
public class LinearSystemSolverTest {
    ...
    void throwingMatrixNotSquare() {
        ...
        assertThrows(LinearAlgebra.MatrixNotSquareException.class, () -> solver.execute(new MatrixVariable(a), null));
        assertThrows(LinearAlgebra.MatrixNotSquareException.class, () -> solver.execute(new MatrixVariable(b), null));
        assertThrows(LinearAlgebra.MatrixNotSquareException.class, () -> solver.execute(new MatrixVariable(c), null));
    }
    ...
    void properlySolvedLinearSystems() {
        ...
        MatrixVariable x = solver.execute(C, D);
        assertEquals(sol1, solver.execute(A, B));
        for (int i = 0; i < 3; i++) {
